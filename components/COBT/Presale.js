...
export default function Presale({buyInPresale, claimPresale, handleBuyAmount, isPresaleEnded, isJoinedPresale, isPresale, isWhiteListed, totalBuyAmount, isOpenForPublic, isClaimingPresale, isClaimedPresaleToken, theHardCap}) {
        ...
        buyInPresale(),
         {
           ...
           success: (res) => (
            <h1 className={styles.toastTry}>Your Transction Hash <a target="_blank" rel="noreferrer" href={`https://bscscan.com/tx/${res}`}>{res}</a></h1>
           ),
           error: (err) => `${err}`,
         },
         {
    ...
    useEffect(()=> {
      setBarPercentage(totalBuyAmount /theHardCap * 100)
    }, [totalBuyAmount])
      ...
      <div className={styles.container}>
        ...
        {!isPresale && !isPresaleEnded && !isOpenForPublic && !isClaimingPresale &&(<><h1 className={styles.title}>Presale Start In:</h1><CountDownTimer d={d} h={h} m={m} s={s} /><div className={styles.info}>
          ...
          <div className={styles.bnbContainer}>
            ...
            <h2 className={styles.bnbMax}>{theHardCap} BNB</h2>
          </div>
        </div>
